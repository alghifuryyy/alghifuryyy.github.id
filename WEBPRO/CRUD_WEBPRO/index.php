...
<body>
    ...
    <?php
    // --- koneksi ke database
    $koneksi = mysqli_connect("localhost", "root", "", "crud_1303190087") or die(mysqli_error());
    ...
    function tambah($koneksi)
    {
        ...
        if (isset($_POST['btn_simpan'])) {
            ...
            if (!empty($nm_tanaman) && !empty($hasil) && !empty($lama) && !empty($tgl_panen)) {
                $sql = "INSERT INTO crud (id,tanaman, hasil, durasi, tgl_panen) VALUES(" . $id . ",'" . $nm_tanaman . "','" . $hasil . "','" . $lama . "','" . $tgl_panen . "')";
    ...
    function ubah($koneksi)
    {
        ...
        if (isset($_POST['btn_ubah'])) {
            ...
            if (!empty($nm_tanaman) && !empty($hasil) && !empty($lama) && !empty($tgl_panen)) {
                ...
                $sql_update = "UPDATE crud SET " . $perubahan . " WHERE id=$id";
    ...
    function hapus($koneksi)
    {
        if (isset($_GET['id']) && isset($_GET['aksi'])) {
            $id = $_GET['id'];
            $sql_hapus = "DELETE FROM crud WHERE id=" . $id;
